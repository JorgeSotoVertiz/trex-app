...
function setup(){
  ...
  //crea el Trex
  END = createSprite(windowWidth/2,windowHeight/2,60,15);
  END.addImage(end);
  END.scale=.5;
  END.visible=false;
  RESET = createSprite(windowWidth/2,windowHeight/2+50,20,20);
  ...
  if(touches.leng>0||keyDown("space")&& trex.y>=windowHeight-150 ){
    trex.velocityY = -10;
  jump.play();
    touches=[];
  }    
  ...
  if(frameCount%60==0){
  nube=createSprite(600,200,40,10);
    nube.addImage(nube2);
  nube.velocityX=-6;    
  nube.y=Math.round(random(20,200));
...
{ 
 ...
 cactus=createSprite(400,windowHeight-120,10,40);
